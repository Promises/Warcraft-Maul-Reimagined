...
export class WarcraftMaul {
    ...
    public GameWin(): void {
        if (this.gameLives > 0) {
            settings.Sounds.victorySound.start()
    ...
    private SpamEffects(): boolean {
        ...
        const effect = new Effect('Abilities\\Spells\\Human\\DispelMagic\\DispelMagicTarget.mdl', x, y);
        effect.destroy();
        this.eventQueue.AddLow(() => this.SpamEffects());
        return true;
    }
    ...
    public GameOver(): void {
        this.gameEndTimer = this.mapSettings.GAME_END_TIME;
        this.gameEnded = true;
        settings.Sounds.defeatSound.start()
    ...
    //         const trig: Trigger = Trigger.create();
