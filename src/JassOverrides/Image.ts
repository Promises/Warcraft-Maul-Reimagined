...
export type ImageColour = { red: uint8, green: uint8, blue: uint8, alpha: uint8 };

/** Textures used for images. */
export const IMAGE_TEXTURES = {
    placementSplat: 'ReplaceableTextures\\Splats\\SuggestedPlacementSplat.blp',
} as const;

export class Image {
    readonly img: image;
    private _colour: ImageColour = {red: 0, alpha: 255, blue: 0, green: 0};
    ...
    constructor(imagePath: string, size: number, x: number, y: number, z: number) {
        ...
        if (GetHandleId(this.img) === -1) {
            // Only happens when the map is loaded in a way that does not mount its imports
            Log.Error(`CreateImage failed for ${imagePath}`);
        }
    }
    ...
    public set colour(col: ImageColour) {
        this._colour = col;
